...
for i in range(len(ncs)):
    matypred.append(numpy.array(ccam.read_csv(old_PLS_path+"\\matypred_"+which_elem[i]+'.csv',0,labelrow=False,skipsym='#')[:,ncs[i]-1],dtype='float'))
    matypres.append(numpy.array(ccam.read_csv(old_PLS_path+"\\matypres_"+which_elem[i]+'.csv',0,labelrow=False,skipsym='#')[:,ncs[i]-1],dtype='float'))
    truecomps.append(numpy.add(matypred[i],matypres[i]))
    


#Read in submodel PLS new db results
new_PLS_path="C:\\Users\\rbanderson\\Documents\\Projects\\MSL\\ChemCam\\DataProcessing\\Working\\Recreating May Results\\ccam_20150515\\Output\\db_predictions_May_results"
newPLS_results=[]
newPLS_samples=[]
newPLS_sample_index=[]
...
for i in range(len(which_elem)):
    ...
    tmp,labels=ccam.read_csv(filelist,3,labelrow=True,skipsym='#')
...
ICA_samples_unique=[]
for i in range(len(which_elem)):
    tmp=ccam.read_csv(ICA_path+'\\'+which_elem[i]+'_ICA.csv',0,labelrow=False,skipsym='#')
...
full_db,labels=ccam.read_csv(full_db_file,0,labelrow=True,skipsym='#')
...
for i in range (len(which_elem)):
    for j in range(len(full_db_unique_samples)):
        ...
        if combined_stdevs_n[j,i]>1: 
            stdtmp=numpy.std(Combined_full[goodindex,i])

            combined_stdevs[j,i]=stdtmp       
        

print 'Writing stdev results'
stdevoutputfile=new_PLS_path+'\\combined_stdevs.csv'
with open(stdevoutputfile,'wb') as writefile:
...
for j in range(len(which_elem)):
    ...
    with open(fulloutfile,'wb') as writefile:
        writer=csv.writer(writefile,delimiter=',')
