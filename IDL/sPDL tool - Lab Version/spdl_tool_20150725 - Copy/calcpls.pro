...
; O. Gasnault 2015 June - For each WHERE function check there in an output of stop the code
...
filen = 'pdl_tool_config.csv'
configdata=rd_tfile(filen,/autocol,delim=',')
sd = where(configdata[0,*] eq 'searchdir', nsd)
if nsd eq 0 then message,'Missing seachdir in '+filen
searchdir=configdata[1,sd]
ml = where(configdata[0,*] eq 'masterlist', nml)
if nml eq 0 then message,'Missing masterlist in '+filen
masterlist=configdata[1,ml]
mf = where(configdata[0,*] eq 'maskfile', nmf)
if nmf eq 0 then message,'Missing maskfile in '+filen
maskfile=configdata[1,mf]
mc = where(configdata[0,*] eq 'meancenters_file', nmc)
if nmc eq 0 then message,'Missing meancenters in '+filen
meancenters_file=configdata[1,mc]
scf = where(configdata[0,*] eq 'settings_coeffs_file', nscf)
if nscf eq 0 then message,'Missing settings_coeffs_file in '+filen
settings_coeffs_file=configdata[1,scf]
bad = where(configdata[0,*] eq 'blend_array_dir', nbad)
if nbad eq 0 then message,'Missing blend_array_dir in '+filen
blend_array_dir=configdata[1,bad]
...
; if not there then inc me
    ...
    if me eq -1 then message,'Missing CALCPLS.'
