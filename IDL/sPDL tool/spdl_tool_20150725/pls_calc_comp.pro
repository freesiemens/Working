...
;   WRITE_RESULTS
;   
; Modification 27 Jun 2015 (OG):
;   PLS_COMP: Add TEST to LABELINDEX to check that the submodel exist (should not happen)
;   PLS_BLEND: Add /NULL to WHERE otherwise K is put to 0 if there is no negative values.
;
...
function pls_comp,currentelem,nshots,which_submodel,ymeancenters,meancenters,pls_settings_labels,pls_norms,pls_coeffs,spectra_norm1,spectra_norm3
        ;get full results
        labelindex=where(pls_settings_labels eq currentelem+'_'+which_submodel, test)
        if test eq 0 then message,'Missing submodel: '+currentelem+'_'+which_submodel
...
function pls_blend,comps,blend_array_dir,elems,filelist
      ...
      blended[where(blended lt 0, /NULL)]=0
      return,blended
end
