function ccam_norm,spectra,wvl,normtype,totals=totals
   ...
   if normtype eq 3 then begin
      uvindex=where(wvl le 340.797, testuv)
      visindex=where(wvl ge 382.138 and wvl le 469.090, testvis)
      vnirindex=where(wvl gt 473.184, testvnir)
      if testuv eq 0 then message,'There is no UV wavelength.'
      if testvis eq 0 then message,'There is no VIS wavelength.'
      if testvnir eq 0 then message,'There is no VNIR wavelength.'
