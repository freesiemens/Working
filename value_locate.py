...
def val_loc_inc(x, u, l64=False):
    ...
    ord1 = numpy.sort(c)
    d   = numpy.append([-1],numpy.array([numpy.where(ord1<nx)]), [nx])
    if y64:
        out=numpy.array(nu,dtype='float64')
    else:
        out=numpy.array(nu,dtype='float32')    
    ...
    for i in range((one-one),(nx-one)+2):
        if nouti[i] >= 1: out[ord1[j0[i]:j1[i]]-nx+1]= i-1
    #check boundaries
    nlow = 1
    itst = 0
    while nlow >= 1:
        ...
        nlow=bound.size
        if nlow>= 1: out[bound]=out[bound]+1 #< (nx-2)
    	#stop
        itst = itst + 1
...
def value_locate(x,u,l64=False):
  ...
  #  else:
    if l64:
        ...
        arrtype='float32'
    out=numpy.zeros(u.shape,dtype=arrtype)

    if x[-1] < x[0]:
        temp=x.size-2-val_loc_inc(x[::-1],u,l64=l64)
    else:
        temp = val_loc_inc(x, u, l64=l64)
